...
public class HealthBarsHooks {
	...
	public void onEntityGetAttackTarget(LivingSetAttackTargetEvent event) {
		if (event.target == null) attackingEntities.remove(event.entity.entityId);
	...
	public void onRenderWorldLast(RenderWorldLastEvent event) {
		...
		if (!Minecraft.isGuiEnabled() || !mod_HealthBars.barsEnabled) return;
		...
		for (Entity entity : ClientUtils.getWorldEntityList(mc.theWorld))
			if (entity != null && entity instanceof EntityLiving && entity.isInRangeToRenderVec3D(renderingVector) && (entity.ignoreFrustumCheck || frustrum.isBoundingBoxInFrustum(entity.boundingBox)) && entity.isEntityAlive()) renderHealthBar((EntityLiving) entity, event.partialTicks, cameraEntity);
	}
	...
	public void renderHealthBar(EntityLiving entity, float partialTicks, Entity viewPoint) {
		...
		if (distance > HealthBarsConfig.maxRenderDistance || !entity.canEntityBeSeen(viewPoint) || entity.getHealth() < HealthBarsConfig.minHealthToRender || entity.getHealth() > HealthBarsConfig.maxHealthToRender || entity instanceof EntityPlayer || HealthBarsConfig.isEntityBlacklisted(CommonUtils.getEntityName(entity))) return;

		if (HealthBarsConfig.renderOnLookOnly && ClientUtils.getEntityLookedAt(partialTicks) != entity) return;
		...
		if (HealthBarsConfig.healthNumbers) renderLabel(entity, String.format("%s/%s (", (double) health / 2, (double) maxHealth / 2) + health * 100 / maxHealth + "%)", (float) (x - RenderManager.renderPosX), (float) (y - RenderManager.renderPosY + entity.height + 0.6), (float) (z - RenderManager.renderPosZ), HealthBarsConfig.maxRenderDistance);
