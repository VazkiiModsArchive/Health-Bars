...
public class HealthBarsTickHandler implements ITickHandler {
	...
	public void tickEnd(EnumSet<TickType> type, Object... tickData) {
		...
		if(type.equals(EnumSet.of(TickType.SERVER))) {
			for (WorldServer world : CommonUtils.getServer().theWorldServer){
	...
	public void sendPackets(EntityTracker tracker){
		...
		while(it.hasNext()){
			...
			if(ticksElapsed % entry.updateFrequency == 0 && entity instanceof EntityLiving){
				tracker.sendPacketToAllPlayersTrackingEntity(entity, getHealthPacket((EntityLiving) entity));
			}
